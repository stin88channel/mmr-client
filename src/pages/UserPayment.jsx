...
function UserPayments() {
  ...
  // Вызов функции редактирования реквизита
  const handleEditPaymentOption = (option) => {
    ...
    // Устанавливаем лимит в состояние, чтобы он отображался в форме
    setLimit(option.limit); // Добавьте это, если у вас есть состояние для лимита
  };
  ...
  const loadPaymentOptions = useCallback(async () => {
    ...
    try {
      ...
      if (cachedData) {
        setPaymentOptions(JSON.parse(cachedData)); // Устанавливаем кэшированные данные
      }
      ...
      if (response.data && Array.isArray(response.data)) {
        setPaymentOptions(response.data); // Устанавливаем данные из ответа
        // Кэшируем новые данные
        localStorage.setItem("paymentOptions", JSON.stringify(response.data));
      } else {
        console.error(
          "Не удалось получить корректные данные с сервера:",
          response.data
        );
      }
    } catch (error) {
      console.error("Ошибка при загрузке реквизитов:", error);
      // Можно добавить уведомление для пользователя о проблеме
      toast.error("Не удалось загрузить платежные опции. Попробуйте позже.");
    } finally {
  ...
  const handleAddPaymentOption = async (e) => {
    ...
    const newOption = {
      ...
      bank: selectedBank,
      limit: Number(formData.get("limit")) || 0, // Убедитесь, что лимит устанавливается правильно
    ...
    // Валидация данных
    const validationError = validatePaymentOption(newOption);
    if (validationError) {
    ...
    try {
      ...
      if (editPaymentOption) {
        ...
        await updatePaymentOption(newOption, form);
      } else {
        // Добавление нового реквизита
        await addPaymentOption(newOption, form);
      }
    } catch (error) {
      console.error("Ошибка при добавлении/редактировании реквизита:", error);
      const errorMessage =
        error.response?.data?.error ||
        error.response?.data?.details ||
        "Не удалось добавить/обновить реквизит. Пожалуйста, попробуйте снова.";
      toast.error(errorMessage);
    } finally {
      // Сбрасываем состояние редактирования и закрываем форму
      setEditPaymentOption(null);
      setShowAddForm(false); // Закрываем форму после добавления/редактирования
                    ...
                    <tbody>
                      {[
                        ...
                        >
                          ...
                          <td>{option.isEmpty ? "" : option.addedBy}</td>
        ...
        >
          ...
          >
            <form onSubmit={handleAddPaymentOption} className="payment_form">
              ...
              <label htmlFor="bot_requisites">Реквизит для бота</label>
              <input
                type="text"
                id="bot_requisites"
                name="bot_requisites"
